document.addEventListener('DOMContentLoaded', function() {
    ...
    // Video modal handling - updated to prevent video cropping
    function openVideoModal() {
        ...
        setTimeout(() => {
            ...
            // Make sure the video container fits properly - revised for full width
            const videoContainer = document.querySelector('.video-container');
            if (videoContainer) {
                // Remove fixed aspect ratio to prevent cropping
                videoContainer.style.padding = '0';
                videoContainer.style.height = 'auto';
                videoContainer.style.width = '100%'; // Explicitly set to 100% width
                videoContainer.style.minHeight = isMobileScreen ? '150px' : (isSmallScreen ? '200px' : '250px');
            ...
            // Ensure video element has proper sizing for full width
            const videoElement = document.getElementById('sectorVideo');
            if (videoElement) {
                videoElement.style.position = 'relative';
                videoElement.style.height = 'auto';
                videoElement.style.width = '100%';
                videoElement.style.maxWidth = 'none'; // Remove max-width constraint
                videoElement.style.maxHeight = isMobileScreen ? '70vh' : (isSmallScreen ? '75vh' : '85vh');
                videoElement.style.objectFit = 'contain'; // Critical for aspect ratio
                videoElement.style.margin = '0 auto'; // Center the video
                // Ensure playsinline attribute is set for mobile devices
