    ...
    adjustModalScale: function(scale) {
        ...
        if (videoModal && videoModalContent) {
            ...
            if (videoModal.style.display === 'block') {
                ...
                // Fix video container to maximize video size
                const videoContainer = document.querySelector('.video-container');
                if (videoContainer) {
                    // Remove fixed aspect ratio to prevent cropping
                    videoContainer.style.paddingBottom = '0';
                    videoContainer.style.height = 'auto';
                    videoContainer.style.width = '100%'; // Ensure container takes full width
                    // Minimal vertical space usage
                    videoContainer.style.margin = '0';
                    // Adjust minimum heights based on orientation - increased for larger video
                    videoContainer.style.minHeight = isLandscape ? 
                                                   (isTinyScreen ? '250px' : '300px') : 
                                                   (isTinyScreen ? '300px' : 
                                                   (isMobileScreen ? '350px' : '400px'));
                }
                
                // Adjust video element directly for maximum size
                const videoElement = document.getElementById('sectorVideo');
                if (videoElement) {
                    videoElement.style.position = 'relative';
                    videoElement.style.height = 'auto';
                    videoElement.style.width = '100%';
                    videoElement.style.maxWidth = 'none'; // Remove any max-width constraint
                    videoElement.style.objectFit = 'contain'; // Ensure aspect ratio is maintained
                    
                    // Maximize height for better video viewing based on orientation
                    videoElement.style.maxHeight = isLandscape ? 
                                                  '98vh' : // Almost full viewport height in landscape
                                                  '98vh';  // Almost full viewport height in portrait
                    
                    // Add additional styles for better mobile fit
